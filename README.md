...
`test-esp32-idf` then run the three builds in parallel against that exact image. On
...
```
docker buildx build --load -t pio-test:local .
docker run --rm pio-test:local /opt/tests/run-all-local.sh
```

For a faster local smoke test on Apple Silicon, `Dockerfile.test.arm64` is a leaner
variant (platform-pinned, without the inline esp32/esp8266 warm-builds):
